...
import { Plus, FolderGit2, Github, Search } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-h2 text-text-primary mb-1">Repositories</h1>
          <p className="text-body text-text-tertiary">
            {repositories.length > 0
              ? `${repositories.length} connected repositor${repositories.length === 1 ? 'y' : 'ies'}`
      ...
      {repositories.length > 0 && (
        <div className="relative max-w-xs">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm bg-surface-3 border border-border-medium rounded-lg text-text-primary placeholder-text-quaternary outline-none focus:border-brand-500 focus:shadow-focus-ring transition-all"
          />
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="card p-14 text-center animate-fade-in">
          ...
          <p className="text-body text-text-tertiary max-w-sm mx-auto mb-7">
            Connect your first GitHub repository to start analyzing your codebase with DevIntel AI.
          </p>
          <button
            onClick={() => setIsConnectModalOpen(true)}
        ...
        <div className="card p-8 text-center">
          <p className="text-sm text-text-tertiary">No repositories match "{filter}"</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredRepos.map((repo: Repository, index: number) => (
            ...
            >
              <RepositoryCard
                ...
                healthReport={healthScores.data?.[repo.id]}
              />
            </div>
          ))}
        </div>
